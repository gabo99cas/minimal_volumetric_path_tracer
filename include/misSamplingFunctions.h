...
//se muestrea BDSF para obtener una dirección para continuar (es agnostico a la fuente, puede darle a cualquiera)
	if(obj.material==0){
...
//MIS con adecuaciones para incluir el efecto de la transmitancia
inline Color MISv2(Sphere &obj, Point x, Vector n, Vector wray, double alpha, const double sigma_t){ //for each ligth source
	Color f, g, montecarlo = Color();
	Vector wiLight, wiBDRF, wo;
	double wf, wg, fpdf, gpdf, costhetaMax;
	int sourceid, sourceid2=0;
...
//se muestrea each light source
	...
	for(int light = 0; light<size; light++) {
		if(spheres[light].r>0 && spheres[light].radiance.x>0){
			f = muestreoSA(spheres[light], x, light, obj, n, wray, wiLight, costhetaMax, alpha) * transmitance(x, spheres[light].p, sigma_t);
			fpdf = solidAngleProb(costhetaMax);
			if(obj.material==0) gpdf = hemiCosineProb(n.dot(wiLight));
			else if(obj.material==2){
			//calcular el fresnel para la direccion muestreada por luz prob = F en reflexion
